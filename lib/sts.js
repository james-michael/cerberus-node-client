...
const getAuthenticationHeaders = async (region) => {
  ...
  try {
    log("No credentials yet. Attempting fetchAwsCredentials")
    awsCredentials = await fetchAwsCredentials()()
  } catch (error) {
    log("------ Error on fetchAwsCredentials")
    log(error.message)
    log("------")
  ...
  try {
    ...
    if (!awsCredentials) {
      log("No credentials yet. Attempting getDefaultRoleAssumerWithWebIdentity")
      awsCredentials = await fromTokenFile({
        roleAssumerWithWebIdentity: getDefaultRoleAssumerWithWebIdentity()
      })()
    }
  } catch (error) {
    log("------ Error on getDefaultRoleAssumerWithWebIdentity")
    log(error.message)
    log("------")
    credsErrors.push(error)
  }
